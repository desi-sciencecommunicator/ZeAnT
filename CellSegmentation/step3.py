def mask_volumes(parameters):
    ...
    if not os.path.isfile(fullname_timemean+hdf):
        ...
        for color_i in range(p.n_colors):
            ...
            # Replace this line below with new section
            # mean_timeseries_raw[color_i] = volume_nameRDD.map(mean_volume).collect()
            
            # With:
            means = []
            for name_volume in p.volume_names:
                fullname_volume = os.path.join(dir_volume, name_volume)
                means.append(np.mean(load_volume(fullname_volume + ali + hdf), dtype='float'))
            mean_timeseries_raw[color_i] = np.array(means)
    ...
    for color_i in range(p.n_colors):
        ...
        # Replacing above section with this new section to override the built in parallelism
        # Including this for loop to determine the volume with the p.type_mask property
        
        volume_mean = np.zeros((lx, ly, lz), dtype='float')
        ...
        for name_volume in p.volume_names[timepoints]:
            ...
            if p.type_mask == 'geomean':
                volume = np.log10(volume)
            if p.type_mask == 'max':
                np.maximum(volume_mean, volume, out=volume_mean)
            else:
                volume_mean += volume
