def detect_cells(parameters):
    ...
    for color_i in range(p.n_colors):
        ...
        def detect_cells_block(ii, xyz0, xyz1):
            # import os
            ...
            voxel_xyz, voxel_timeseries, peak_idx, voxel_similarity_peak = \
                process_block_data(xyz0, xyz1, parameters, color_i, lxyz, rxyz,
                                   ball_diam, volume_mean, volume_peak, timepoints) #replaced variables with direct values volume_mean, volume_peak
            ...
            with h5py.File(fullname_block+hdf, 'w') as file_handle:
                if success:
                    for ci in range(n_cells):
                        ...
                        bi = np.sum(wi * volume_mean[tuple(zip(*xyzi))]) / np.sum(wi) #replaced variable with direct value of volume_mean
        ...
        if block_valids.any():
            ...
            for item in block_ixyz01:
                detect_cells_block(*item)
