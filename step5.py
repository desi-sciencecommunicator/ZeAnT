def clean_cells(parameters):
    ...
    # REMOVED: pyspark imports, SparkSession, sc, evenly_parallelize import
    ...
    for color_i in range(p.n_colors):
        ...
        # REMOVED: bparameters = sc.broadcast(parameters)

        def get_timebase(timeseries):          # takes timeseries directly, not a tuple
            return clean_signal(parameters, timeseries)   # parameters directly, not bparameters.value

        # REMOVED: if p.parallel_clean branch
        # Replace the whole if/else with:
        print('Computing baseline in serial mode... ', end='')
        timebase = [get_timebase(ts) for ts in cell_timeseries]
        print('done.')
